...
var _Function = _interopRequireWildcard(require("effect/Function"));
...
/** @internal */
const xForwardedHeaders = exports.xForwardedHeaders = /*#__PURE__*/make(httpApp => Effect.updateService(httpApp, ServerRequest.HttpServerRequest, request => {
...
const cors = options => {
  ...
  const allowOrigin = typeof opts.allowedOrigins === "function" || opts.allowedOrigins.length > 1 ? originHeader => {
    ...
    return undefined;
  } : opts.allowedOrigins.length === 1 ? Function.constant({
