...
export class Button extends AbstractButton {
    static displayName = `${DISPLAYNAME_PREFIX}.Button`;
    // need to keep this ref so that we can access it in AbstractButton#handleKeyUp
    buttonRef = null;
    handleRef = refHandler(this, "buttonRef", this.props.elementRef);
    render() {
        return (React.createElement("button", { type: "button", ref: this.handleRef, ...removeNonHTMLProps(this.props), ...this.getCommonButtonProps() }, this.renderChildren()));
    }
...
export class AnchorButton extends AbstractButton {
    static displayName = `${DISPLAYNAME_PREFIX}.AnchorButton`;
    // need to keep this ref so that we can access it in AbstractButton#handleKeyUp
    buttonRef = null;
    handleRef = refHandler(this, "buttonRef", this.props.elementRef);
    render() {
        ...
        return (React.createElement("a", { role: "button", ref: this.handleRef, ...removeNonHTMLProps(this.props), ...commonProps, href: commonProps.disabled ? undefined : href, tabIndex: commonProps.disabled ? -1 : tabIndex }, this.renderChildren()));
    }
    componentDidUpdate(prevProps) {
        if (prevProps.elementRef !== this.props.elementRef) {
            setRef(prevProps.elementRef, null);
