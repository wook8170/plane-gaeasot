...
}).pipe(Effect.provideService(IncomingMessage.MaxBodySize, Option.some(FileSystem.Size(1024 * 1024 * 10))));
/** @internal */
exports.make = make;
    ...
    case "Raw":
      {
        ...
        if (typeof body.body === "object" && body.body !== null && "pipe" in body.body && typeof body.body.pipe === "function") {
          ...
          }).pipe(Effect.interruptible, Effect.tapErrorCause(handleCause(nodeResponse, response)));
        }
        return Effect.async(resume => {
    ...
    case "Stream":
      {
        ...
        return body.stream.pipe(Stream.orDie, Stream.runForEachChunk(chunk => {
...
const handleCause = (nodeResponse, original) => originalCause => Error.causeResponse(originalCause).pipe(Effect.flatMap(([response, cause]) => {
  ...
  return Effect.failCause(headersSent ? Cause.sequential(originalCause, Cause.die(original)) : cause);
}));
/** @internal */
