...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  yield* refreshLocks.pipe(Effect.tapErrorCause(Effect.logWarning), Effect.retry(Schedule.spaced(500)), Effect.scheduleForked(Schedule.fixed(lockRefreshInterval)), Effect.annotateLogs({
    package: "@effect/sql",
  ...
  const mailboxes = yield* RcMap.make({
    lookup: Effect.fnUntraced(function* ({
      ...
      yield* Effect.addFinalizer(() => Effect.flatMap(mailbox.clear, elements => {
  ...
  return PersistedQueue.PersistedQueueStore.of({
    offer: ({
      ...
      id,
      name
    }) => Effect.catchAllCause(Effect.suspend(() => offer(id, name, JSON.stringify(element))), cause => Effect.fail(new PersistedQueue.PersistedQueueError({
    ...
    take: ({
      ...
      onFailure: cause => Cause.isInterruptedOnly(cause) ? interrupt([element.sequence]) : retry(element.sequence, element.attempts + 1, cause),
