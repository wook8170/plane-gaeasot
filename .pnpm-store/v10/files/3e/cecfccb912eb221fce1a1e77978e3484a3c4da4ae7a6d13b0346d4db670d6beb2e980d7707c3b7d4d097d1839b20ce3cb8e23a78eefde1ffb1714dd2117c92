  ...
  return yield* make({
    ...
    send({
      ...
      if (message._tag === "OutgoingEnvelope") {
        ...
        })), Effect.catchTag("RpcClientError", Effect.die), Effect.scoped, Effect.catchAllDefect(() => Effect.fail(new RunnerUnavailable({
          address
        }))));
      }
      ...
      if (!isStream) {
        return Effect.matchEffect(Message.serializeRequest(message), {
          onSuccess: request => RcMap.get(clients, address).pipe(Effect.flatMap(client => client.Effect({
            request,
            persisted: isPersisted
          })), Effect.catchTag("RpcClientError", Effect.die), Effect.flatMap(reply => Schema.decode(Reply.Reply(message.rpc))(reply).pipe(Effect.locally(FiberRef.currentContext, message.context), Effect.orDie)), Effect.flatMap(message.respond), Effect.scoped, Effect.catchAllDefect(() => Effect.fail(new RunnerUnavailable({
            address
          })))),
