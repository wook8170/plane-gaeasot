...
var _ComboboxRootInternal = require("../../combobox/root/ComboboxRootInternal");
var _utils = require("../../combobox/root/utils");
...
function AutocompleteRoot(props) {
  ...
  const handleValueChange = (0, _useEventCallback.useEventCallback)((nextValue, eventDetails) => {
  ...
  const collator = (0, _useFilter.useCoreFilter)(DEFAULT_FILTER_OPTIONS);
  const baseFilter = React.useMemo(() => {
    if (rest.filter) {
      return rest.filter;
    }
    return (item, query, toString) => {
      return collator.contains((0, _utils.stringifyItem)(item, toString), query);
    };
  }, [rest, collator]);
  ...
  const handleItemHighlighted = (0, _useEventCallback.useEventCallback)((highlightedValue, data) => {
    ...
    if (enableInline) {
      if (highlightedValue == null) {
        ...
        setInlineOverlay((0, _utils.stringifyItem)(highlightedValue, itemToStringValue));
      }
    } else {
      setInlineOverlay('');
    }
