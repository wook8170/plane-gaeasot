...
/** @internal */
const retryTransient = exports.retryTransient = /*#__PURE__*/(0, _Function.dual)(2, (self, options) => {
  ...
  return transformResponse(self, (0, _Function.flow)(mode === "errors-only" ? _Function.identity : Effect.repeat({
    schedule: passthroughSchedule,
    times,
    while: isOnlySchedule || options.while === undefined ? isTransientResponse : Predicate.and(isTransientResponse, options.while)
  }), mode === "response-only" ? _Function.identity : Effect.retry({
...
const isTransientHttpError = error => Error.isHttpClientError(error) && (error._tag === "RequestError" && error.reason === "Transport" || error._tag === "ResponseError" && isTransientResponse(error.response));
